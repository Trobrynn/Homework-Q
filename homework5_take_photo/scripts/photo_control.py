    ...
    def image_callback(self, data):
		...
		rospy.sleep(8)
            ...
            if len(faces_result)>(self.faces_last_result) and self.face_pub==False:
                facesstring="I found %d faces."%(len(faces_result))
                self.face_pub=True
            ...
            # 在opencv的窗口中框出所有人脸区域
                if len(faces_result)>0:
                    ...
                    rospy.loginfo(mid_y)
            # 根据脸的位置发布速度消息
                    if  (mid_y<220 and 320<mid_x and mid_x<360):
                        strings="Please step back a little"

            # 后退
                    elif (mid_y>260 and 320<mid_x and mid_x<360):
                        strings="Please move forward a little"


            # 停止
                    elif (mid_y>=220 and mid_y<=260 and 320<=mid_x and mid_x<=360):
